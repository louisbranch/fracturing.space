...
Track unresolved mechanics directly in this timeline contract using explicit
`TBD` command/event rows and clarification-gate notes until the contract is
resolved.
...
sub-events and optional `countdown_advanced` events, all batch-appended in a
single call.

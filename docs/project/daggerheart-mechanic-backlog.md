---
title: "Daggerheart Mechanic Backlog"
parent: "Project"
nav_order: 24
---

# Daggerheart Mechanic Backlog

Priority mechanics backlog for Daggerheart. Each row maps a missing mechanic
to the evented write path. This document evolves as mechanics are implemented
or clarified.

For the stable reference (implemented mechanics, handler rules, templates,
patterns), see
[Daggerheart Event Timeline Contract](daggerheart-event-timeline-contract.md).

For exact payload fields and emitter references, use:
...
resolved. For the general pattern, see
[Clarification gate pattern](game-systems.md#clarification-gate-pattern).
...
**P31** (all sub-scenarios) — see P31 Clarification Gate above.

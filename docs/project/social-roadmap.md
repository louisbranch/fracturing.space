...
Use a dedicated `social` service boundary (not `people`).

- Canonical entity term remains `User`.
- `auth` remains authN/authZ focused (proof of identity and access artifacts).
- `social` owns social/discovery and relationship metadata.

Boundary rubric:

1. AuthN/authZ artifacts belong to `auth`.
2. User discovery/recognition/relationship artifacts belong to `social`.
...
- `social`: contact graph and social identity fields in one `UserProfile` aggregate (`username`, `name`, `avatar_set_id`, `avatar_asset_id`, `bio`), plus future connection request lifecycle and connection links.
...
## Phase 1 (Completed): Stand Up `social` + Migrate Contacts

### Goals

- Create a new `social` service boundary.
- Move contact ownership from `auth` to `social`.
...
- One user-owned social/discovery profile aggregate in `social`.

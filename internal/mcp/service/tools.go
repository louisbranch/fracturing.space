...
func registerCampaignTools(mcpServer *mcp.Server, client campaignv1.CampaignServiceClient, getContext func() domain.Context, notify domain.ResourceUpdateNotifier) {
	mcp.AddTool(mcpServer, domain.CampaignCreateTool(), domain.CampaignCreateHandler(client, notify))
	mcp.AddTool(mcpServer, domain.ParticipantCreateTool(), domain.ParticipantCreateHandler(client, notify))
	mcp.AddTool(mcpServer, domain.CharacterCreateTool(), domain.CharacterCreateHandler(client, notify))
	mcp.AddTool(mcpServer, domain.CharacterControlSetTool(), domain.CharacterControlSetHandler(client, notify))
	mcp.AddTool(mcpServer, domain.CharacterSheetGetTool(), domain.CharacterSheetGetHandler(client, getContext))
	mcp.AddTool(mcpServer, domain.CharacterProfilePatchTool(), domain.CharacterProfilePatchHandler(client, getContext, notify))
	mcp.AddTool(mcpServer, domain.CharacterStatePatchTool(), domain.CharacterStatePatchHandler(client, getContext, notify))
}

func registerSessionTools(mcpServer *mcp.Server, client sessionv1.SessionServiceClient, getContext func() domain.Context, notify domain.ResourceUpdateNotifier) {
	...
	mcp.AddTool(mcpServer, domain.SessionActionRollTool(), domain.SessionActionRollHandler(client, getContext, notify))
	mcp.AddTool(mcpServer, domain.SessionRollOutcomeApplyTool(), domain.SessionRollOutcomeApplyHandler(client, getContext, notify))
}

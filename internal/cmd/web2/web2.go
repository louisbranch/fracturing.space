// Package web2 parses command config and boots the web2 service.
package web2
...
import (
	...
	"github.com/louisbranch/fracturing.space/internal/services/web2"
)

// Config holds command inputs for web2 startup.
type Config struct {
	HTTPAddr                  string        `env:"FRACTURING_SPACE_WEB2_HTTP_ADDR" envDefault:"localhost:8092"`
	ChatHTTPAddr              string        `env:"FRACTURING_SPACE_CHAT_HTTP_ADDR" envDefault:"localhost:8086"`
	EnableExperimentalModules bool          `env:"FRACTURING_SPACE_WEB2_ENABLE_EXPERIMENTAL_MODULES" envDefault:"false"`
	...
	GRPCDialTimeout           time.Duration `env:"FRACTURING_SPACE_WEB2_DIAL_TIMEOUT" envDefault:"2s"`
}
...
func ParseConfig(fs *flag.FlagSet, args []string) (Config, error) {
	...
	fs.BoolVar(&cfg.EnableExperimentalModules, "enable-experimental-modules", cfg.EnableExperimentalModules, "Enable experimental web2 module surfaces")
...
// Run starts the web2 service process.
func Run(ctx context.Context, cfg Config) error {
	return entrypoint.RunWithTelemetry(ctx, entrypoint.ServiceWeb2, func(context.Context) error {
		...
		server, err := web2.NewServer(ctx, web2.Config{HTTPAddr: cfg.HTTPAddr, AssetBaseURL: cfg.AssetBaseURL, ChatHTTPAddr: cfg.ChatHTTPAddr, EnableExperimentalModules: cfg.EnableExperimentalModules, CampaignClient: statev1.NewCampaignServiceClient(gameConn), ParticipantClient: statev1.NewParticipantServiceClient(gameConn), CharacterClient: statev1.NewCharacterServiceClient(gameConn), SessionClient: statev1.NewSessionServiceClient(gameConn), InviteClient: statev1.NewInviteServiceClient(gameConn), AuthClient: authv1.NewAuthServiceClient(authConn), AccountClient: authv1.NewAccountServiceClient(authConn), ConnectionsClient: connectionsv1.NewConnectionsServiceClient(connectionsConn), CredentialClient: aiv1.NewCredentialServiceClient(aiConn)})
		if err != nil {
			return fmt.Errorf("init web2 server: %w", err)
		}
		defer server.Close()
		if err := server.ListenAndServe(ctx); err != nil {
			return fmt.Errorf("serve web2: %w", err)
		}
		return nil
	})
}

...
// EventService implements the state.v1.EventService gRPC API.
type EventService struct {
	statev1.UnimplementedEventServiceServer
	stores Stores
}
...
func (s *EventService) AppendEvent(ctx context.Context, in *statev1.AppendEventRequest) (*statev1.AppendEventResponse, error) {
	if in == nil {
	...
	return &statev1.AppendEventResponse{Event: eventToProto(stored)}, nil
}

// ListEvents returns a paginated, filtered, and sorted list of events for a campaign.
func (s *EventService) ListEvents(ctx context.Context, in *statev1.ListEventsRequest) (*statev1.ListEventsResponse, error) {
	if in == nil {
	...
	// Build response
	response := &statev1.ListEventsResponse{
		Events:    make([]*statev1.Event, 0, len(result.Events)),
...
func eventToProto(evt event.Event) *statev1.Event {
	return &statev1.Event{

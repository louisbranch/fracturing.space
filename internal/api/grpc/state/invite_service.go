...
// InviteService implements the state.v1.InviteService gRPC API.
type InviteService struct {
	statev1.UnimplementedInviteServiceServer
	stores      Stores
...
func (s *InviteService) CreateInvite(ctx context.Context, in *statev1.CreateInviteRequest) (*statev1.CreateInviteResponse, error) {
	if in == nil {
	...
	return &statev1.CreateInviteResponse{Invite: inviteToProto(created)}, nil
}

// GetInvite returns an invite by ID.
func (s *InviteService) GetInvite(ctx context.Context, in *statev1.GetInviteRequest) (*statev1.GetInviteResponse, error) {
	if in == nil {
	...
	return &statev1.GetInviteResponse{Invite: inviteToProto(inv)}, nil
}

// ListInvites returns a page of invites for a campaign.
func (s *InviteService) ListInvites(ctx context.Context, in *statev1.ListInvitesRequest) (*statev1.ListInvitesResponse, error) {
	if in == nil {
	...
	response := &statev1.ListInvitesResponse{NextPageToken: page.NextPageToken}
	if len(page.Invites) == 0 {
		return response, nil
	}

	response.Invites = make([]*statev1.Invite, 0, len(page.Invites))
...
func (s *InviteService) RevokeInvite(ctx context.Context, in *statev1.RevokeInviteRequest) (*statev1.RevokeInviteResponse, error) {
	if in == nil {
	...
	return &statev1.RevokeInviteResponse{Invite: inviteToProto(inv)}, nil
}

...
func (h handlers) handleRoot(w http.ResponseWriter, r *http.Request) {
	...
	copy := web2i18n.Auth(langTag)
	h.writeAuthPage(w, r, copy.LandingTitle, copy.MetaDescription, langTag.String(), web2templates.PublicRootFragment(copy))
}
...
func (h handlers) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	langTag := h.resolveAuthTag(w, r)
	copy := web2i18n.Auth(langTag)
	h.writeAuthPage(w, r, copy.LoginTitle, copy.MetaDescription, langTag.String(), web2templates.PasskeyLoginPage(copy))
}
...
func (h handlers) writeAuthPageWithStatus(w http.ResponseWriter, r *http.Request, title string, metaDesc string, lang string, statusCode int, body templ.Component) {
	...
	if err := web2templates.AuthLayout(title, metaDesc, lang, r.URL.Path, r.URL.RawQuery).Render(ctx, &rendered); err != nil {
...
func (h handlers) writeNotFoundPage(w http.ResponseWriter, r *http.Request) {
	loc, lang := web2i18n.ResolveLocalizer(w, r, nil)
	h.writeAuthPageWithStatus(
		w,
		r,
		web2templates.AppErrorPageTitle(http.StatusNotFound, loc),
		web2templates.T(loc, "layout.meta_description"),
		lang,
		http.StatusNotFound,
		web2templates.AppErrorState(http.StatusNotFound, loc),
	)
}
...
func (h handlers) writeJSONError(w http.ResponseWriter, r *http.Request, err error) {
	loc, _ := web2i18n.ResolveLocalizer(w, r, nil)

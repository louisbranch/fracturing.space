...
func TestFeatureModulesDoNotImportSiblingModules(t *testing.T) {
	...
	for _, file := range entries {
		...
		for _, imp := range parsed.Imports {
			...
			if strings.Contains(path, "/internal/services/web2/modules/") {
...
func TestCampaignsAndSettingsMountDoNotReadGatewayClientsFromDependencies(t *testing.T) {
	...
	// TODO(web2-guardrails): generalize this guard so new modules cannot silently bypass composition-owned gateway wiring rules.

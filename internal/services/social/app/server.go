...
import (
	...
	socialservice "github.com/louisbranch/fracturing.space/internal/services/social/api/grpc/social"
...
type serverEnv struct {
	DBPath string `env:"FRACTURING_SPACE_SOCIAL_DB_PATH"`
}
...
func loadServerEnv() serverEnv {
	...
	if strings.TrimSpace(cfg.DBPath) == "" {
		cfg.DBPath = filepath.Join("data", "social.db")
	}
	return cfg
}

// Server hosts the social gRPC API and storage lifecycle.
type Server struct {
...
func NewWithAddr(addr string) (*Server, error) {
	...
	store, err := openSocialStore(srvEnv.DBPath)
	if err != nil {
		_ = listener.Close()
		return nil, err
	}
	...
	apiService := socialservice.NewService(store)
	healthServer := health.NewServer()
	socialv1.RegisterSocialServiceServer(grpcServer, apiService)
	grpc_health_v1.RegisterHealthServer(grpcServer, healthServer)
	healthServer.SetServingStatus("", grpc_health_v1.HealthCheckResponse_SERVING)
	healthServer.SetServingStatus("social.v1.SocialService", grpc_health_v1.HealthCheckResponse_SERVING)
...
func (s *Server) Serve(ctx context.Context) error {
	...
	log.Printf("social server listening at %v", s.listener.Addr())
...
// Close releases social server resources.
func (s *Server) Close() {
	if s == nil {
		return
	}
...
func openSocialStore(path string) (*socialsqlite.Store, error) {
	...
	store, err := socialsqlite.Open(path)
	if err != nil {
		return nil, fmt.Errorf("open social sqlite store: %w", err)
	}
	return store, nil
}

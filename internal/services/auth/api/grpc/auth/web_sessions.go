...
func (s *AuthService) CreateWebSession(ctx context.Context, in *authv1.CreateWebSessionRequest) (*authv1.CreateWebSessionResponse, error) {
	...
	session, err := s.createWebSession(ctx, userID, ttl)
	if err != nil {
		return nil, err
	}
...
func (s *AuthService) createWebSession(ctx context.Context, userID string, ttl time.Duration) (storage.WebSession, error) {
	if s == nil || s.webSessionStore == nil {
		return storage.WebSession{}, status.Error(codes.Internal, "Web session store is not configured.")
	}
	session, err := s.buildWebSession(userID, ttl)
	if err != nil {
		return storage.WebSession{}, err
	}
	if err := s.webSessionStore.PutWebSession(ctx, session); err != nil {
		return storage.WebSession{}, status.Errorf(codes.Internal, "Put web session: %v", err)
	}
	return session, nil
}

func (s *AuthService) buildWebSession(userID string, ttl time.Duration) (storage.WebSession, error) {
	if s == nil {
		return storage.WebSession{}, status.Error(codes.Internal, "Auth service is required.")
	}
	id, err := s.idGenerator()
	if err != nil {
		return storage.WebSession{}, status.Errorf(codes.Internal, "Generate web session ID: %v", err)
	}
	now := s.clock().UTC()
	return storage.WebSession{ID: id, UserID: userID, CreatedAt: now, ExpiresAt: now.Add(ttl)}, nil
}

...
func (s *DirectSession) daggerheartAttackFlowResolve(ctx context.Context, argsJSON []byte) (orchestration.ToolResult, error) {
	...
	boardPayload, err := s.loadDaggerheartCombatBoardPayload(callCtx, campaignID, sessionID)
	if err != nil {
		return orchestration.ToolResult{}, err
	}
	...
	profile, err := s.resolveAttackProfile(callCtx, campaignID, characterID, input.StandardAttack, input.BeastformAttack)
	if err != nil {
		return orchestration.ToolResult{}, err
	}
	...
	resp, err := s.clients.Daggerheart.SessionAttackFlow(callCtx, req)
	if err != nil {
...
func (s *DirectSession) daggerheartAdversaryAttackFlowResolve(ctx context.Context, argsJSON []byte) (orchestration.ToolResult, error) {
	...
	resp, err := s.clients.Daggerheart.SessionAdversaryAttackFlow(callCtx, &pb.SessionAdversaryAttackFlowRequest{
		CampaignId:              campaignID,
		SessionId:               sessionID,
		SceneId:                 sceneID,
...
func (s *DirectSession) daggerheartGroupActionFlowResolve(ctx context.Context, argsJSON []byte) (orchestration.ToolResult, error) {
	...
	resp, err := s.clients.Daggerheart.SessionGroupActionFlow(callCtx, &pb.SessionGroupActionFlowRequest{
		CampaignId:        campaignID,
		SessionId:         sessionID,
		SceneId:           sceneID,
...
func (s *DirectSession) daggerheartTagTeamFlowResolve(ctx context.Context, argsJSON []byte) (orchestration.ToolResult, error) {
	...
	resp, err := s.clients.Daggerheart.SessionTagTeamFlow(callCtx, &pb.SessionTagTeamFlowRequest{
		CampaignId:          campaignID,
		SessionId:           sessionID,
		SceneId:             sceneID,
		First:               first,
...
func (s *DirectSession) daggerheartReactionFlowResolve(ctx context.Context, argsJSON []byte) (orchestration.ToolResult, error) {
	...
	resp, err := s.clients.Daggerheart.SessionReactionFlow(callCtx, &pb.SessionReactionFlowRequest{
		CampaignId:           campaignID,
		SessionId:            sessionID,
		SceneId:              sceneID,
		CharacterId:          characterID,
...
func attackOutcomeSummaryFromProto(resp *pb.DaggerheartApplyAttackOutcomeResponse) *attackOutcomeSummary {
	...
	if result := resp.GetResult(); result != nil {
		summary.Result = &attackOutcomeResultSummary{
			Outcome: result.GetOutcome().String(),
			Success: result.GetSuccess(),
			Crit:    result.GetCrit(),
			Flavor:  strings.TrimSpace(result.GetFlavor()),
		}
	}
	return summary
}
...
func adversaryAttackOutcomeSummaryFromProto(resp *pb.DaggerheartApplyAdversaryAttackOutcomeResponse) *adversaryAttackOutcomeSummary {
	...
	summary := &adversaryAttackOutcomeSummary{
		RollSeq:     resp.GetRollSeq(),
		AdversaryID: strings.TrimSpace(resp.GetAdversaryId()),
		Targets:     compactStrings(resp.GetTargets()),
	}
	if result := resp.GetResult(); result != nil {
		summary.Result = &adversaryAttackOutcomeResultSummary{
			Success:    result.GetSuccess(),
			Crit:       result.GetCrit(),
			Roll:       int(result.GetRoll()),
			Total:      int(result.GetTotal()),
			Difficulty: int(result.GetDifficulty()),
		}
	}
	return summary
}
...
func reactionOutcomeSummaryFromProto(resp *pb.DaggerheartApplyReactionOutcomeResponse) *reactionOutcomeSummary {
	...
	summary := &reactionOutcomeSummary{
		RollSeq:     resp.GetRollSeq(),
		CharacterID: strings.TrimSpace(resp.GetCharacterId()),
	}
	if result := resp.GetResult(); result != nil {
		summary.Result = &reactionOutcomeResultSummary{
			Outcome:            result.GetOutcome().String(),
			Success:            result.GetSuccess(),
			Crit:               result.GetCrit(),
			CritNegatesEffects: result.GetCritNegatesEffects(),
			EffectsNegated:     result.GetEffectsNegated(),
		}
	}
	return summary
}
...
func groupActionSupportersToProto(values []groupActionSupporterInput) ([]*pb.GroupActionSupporter, error) {
	...
	for idx, value := range values {
		...
		if characterID == "" {
			return nil, fmt.Errorf("supporters[%d].character_id is required", idx)
		}
		trait := strings.TrimSpace(value.Trait)
		if trait == "" {
			return nil, fmt.Errorf("supporters[%d].trait is required", idx)
		}
...
func (s *DirectSession) resolveAttackProfile(ctx context.Context, campaignID, characterID string, explicitStandard *standardAttackProfileInput, explicitBeastform *beastformAttackProfileInput) (inferredAttackProfile, error) {
	...
	sheet, err := s.loadCharacterSheetPayload(ctx, campaignID, characterID)
	if err != nil {
		return inferredAttackProfile{}, err
	}
	if beastform := inferredBeastformAttackProfile(sheet, characterID); beastform != nil {
		return *beastform, nil
	}
	if standard := inferredPrimaryWeaponAttackProfile(sheet, characterID); standard != nil {
		return *standard, nil
	}

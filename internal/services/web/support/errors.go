...
// ErrorPageTextKeys provides a normalization map from human-readable to i18n keys.
...
// ErrorPageMessageTextKeys maps raw backend strings to message keys.
...
// LocalizeErrorText resolves a raw text value against a translation map.
func LocalizeErrorText(loc webtemplates.Localizer, raw string, keyMap map[string]string) string {
	key, ok := keyMap[raw]
	if !ok {

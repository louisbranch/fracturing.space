...
// GRPCGateway maps gRPC notifications operations to the app gateway contract.
type GRPCGateway struct {
	Client NotificationClient
}

// NewGRPCGateway builds the production notifications gateway from the notification client.
func NewGRPCGateway(client NotificationClient) notificationsapp.Gateway {
	if client == nil {
		return notificationsapp.NewUnavailableGateway()
	}
...
func (g GRPCGateway) GetNotification(ctx context.Context, userID string, notificationID string) (notificationsapp.NotificationSummary, error) {
	...
	if err != nil {
		return notificationsapp.NotificationSummary{}, apperrors.MapGRPCTransportError(err, apperrors.GRPCStatusMapping{
	...
	if resp == nil {
		return notificationsapp.NotificationSummary{}, apperrors.EK(apperrors.KindUnavailable, "error.web.message.failed_to_get_notification", "failed to get notification")
	}
...
func (g GRPCGateway) listAllNotifications(ctx context.Context, userID string) ([]notificationsapp.NotificationSummary, error) {
	...
	return grpcpaging.CollectPagesMax[notificationsapp.NotificationSummary, *notificationsv1.Notification](
		...
		func(notification *notificationsv1.Notification) (notificationsapp.NotificationSummary, bool) {
			if notification == nil {
				return notificationsapp.NotificationSummary{}, false
			}

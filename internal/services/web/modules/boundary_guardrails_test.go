...
func TestSettingsModuleWiresAccountAndAIAppServicesDirectly(t *testing.T) {
	...
	assertFileContains(t, "settings/app/service_test_helpers_test.go", "func newService(gateway Gateway) service")
}
...
func TestPublicAuthRootWiresAppServiceDirectly(t *testing.T) {
	...
	assertFileContains(t, "publicauth/handlers.go", "func newHandlerServicesFromGateway(gateway publicauthapp.Gateway, authBaseURL string) handlerServices")
	...
	assertFileContains(t, "publicauth/app/service.go", "func NewPageService(gateway Gateway, authBaseURL string) PageService")
	assertFileContains(t, "publicauth/app/service.go", "func NewSessionService(gateway Gateway, authBaseURL string) SessionService")
	assertFileContains(t, "publicauth/app/service.go", "func NewPasskeyService(gateway Gateway, authBaseURL string) PasskeyService")
	assertFileContains(t, "publicauth/app/service.go", "func NewRecoveryService(gateway Gateway, authBaseURL string) RecoveryService")
	assertFileContains(t, "publicauth/handlers_test_helpers_test.go", "func newHandlersFromGateway(")
	assertFileContains(t, "publicauth/app/service_test_helpers_test.go", "func newService(gateway Gateway, authBaseURL string) service")
...
func TestPublicRegistryUsesAreaOwnedCompositionEntrypoints(t *testing.T) {
	...
	assertFileContains(t, "invite/handlers.go", "h.principal.ResolveUserID(r)")
...
func TestCampaignDetailRenderSeamIsAreaOwned(t *testing.T) {
	...
	assertFileContains(t, "campaigns/composition.go", "newServiceConfigFromGRPCDeps(newGatewayDeps(config), config.AssetBaseURL)")
...
func TestCampaignServiceConstructorUsesExplicitCapabilityConfig(t *testing.T) {
	...
	assertFileContains(t, "campaigns/handlers.go", "func newHandlerServices(config campaignapp.ServiceConfig)")
	assertFileContains(t, "campaigns/handlers.go", "participantReads  campaignapp.CampaignParticipantReadService")
	assertFileContains(t, "campaigns/handlers.go", "participantMutate campaignapp.CampaignParticipantMutationService")
	assertFileContains(t, "campaigns/handlers.go", "automationReads   campaignapp.CampaignAutomationReadService")
	assertFileContains(t, "campaigns/handlers.go", "automationMutate  campaignapp.CampaignAutomationMutationService")
	assertFileContains(t, "campaigns/handlers.go", "characterReads    campaignapp.CampaignCharacterReadService")
	assertFileContains(t, "campaigns/handlers.go", "characterControl  campaignapp.CampaignCharacterControlService")
	assertFileContains(t, "campaigns/handlers.go", "characterMutate   campaignapp.CampaignCharacterMutationService")
	assertFileContains(t, "campaigns/handlers.go", "sessionReads      campaignapp.CampaignSessionReadService")
	assertFileContains(t, "campaigns/handlers.go", "sessionMutate     campaignapp.CampaignSessionMutationService")
	assertFileContains(t, "campaigns/handlers.go", "inviteReads       campaignapp.CampaignInviteReadService")
	assertFileContains(t, "campaigns/handlers.go", "inviteMutate      campaignapp.CampaignInviteMutationService")
	assertFileContains(t, "campaigns/handlers.go", "ParticipantReads   campaignapp.CampaignParticipantReadService")
	assertFileContains(t, "campaigns/handlers.go", "ParticipantMutate  campaignapp.CampaignParticipantMutationService")
	assertFileContains(t, "campaigns/handlers.go", "AutomationReads    campaignapp.CampaignAutomationReadService")
	assertFileContains(t, "campaigns/handlers.go", "AutomationMutation campaignapp.CampaignAutomationMutationService")
	assertFileContains(t, "campaigns/handlers.go", "SessionReads       campaignapp.CampaignSessionReadService")
	assertFileContains(t, "campaigns/handlers.go", "SessionMutation    campaignapp.CampaignSessionMutationService")
	assertFileContains(t, "campaigns/handlers.go", "InviteReads        campaignapp.CampaignInviteReadService")
	assertFileContains(t, "campaigns/handlers.go", "InviteMutation     campaignapp.CampaignInviteMutationService")
	assertFileContains(t, "campaigns/handlers.go", "CreationPages      campaignworkflow.PageAppService")
	assertFileContains(t, "campaigns/handlers.go", "CreationFlow       campaignworkflow.MutationAppService")
	assertFileContains(t, "campaigns/handlers.go", "campaignapp.NewParticipantReadService(config.ParticipantRead, config.Authorization)")
	assertFileContains(t, "campaigns/handlers.go", "campaignapp.NewParticipantMutationService(config.ParticipantMutation, config.Authorization)")
	assertFileContains(t, "campaigns/handlers.go", "campaignapp.NewAutomationReadService(config.AutomationRead, config.Authorization)")
	assertFileContains(t, "campaigns/handlers.go", "campaignapp.NewAutomationMutationService(config.AutomationMutation, config.Authorization)")
	assertFileContains(t, "campaigns/handlers.go", "campaignapp.NewCharacterReadService(config.CharacterRead, config.Authorization)")
	assertFileContains(t, "campaigns/handlers.go", "campaignapp.NewCharacterControlService(config.CharacterControl, config.Authorization)")
	assertFileContains(t, "campaigns/handlers.go", "campaignapp.NewCharacterMutationService(config.CharacterMutation, config.Authorization)")
	assertFileContains(t, "campaigns/handlers.go", "campaignapp.NewSessionReadService(config.SessionRead)")
	assertFileContains(t, "campaigns/handlers.go", "campaignapp.NewSessionMutationService(config.SessionMutation, config.Authorization)")
	assertFileContains(t, "campaigns/handlers.go", "campaignapp.NewInviteReadService(config.InviteRead, config.Authorization)")
	assertFileContains(t, "campaigns/handlers.go", "campaignapp.NewInviteMutationService(config.InviteMutation, config.Authorization)")
	assertFileDoesNotContain(t, "campaigns/handlers.go", "func newHandlersFromService(")
	assertFileContains(t, "campaigns/composition.go", "serviceConfig := newServiceConfigFromGRPCDeps(newGatewayDeps(config), config.AssetBaseURL)")
	assertFileContains(t, "campaigns/composition.go", "Services:         newHandlerServices(serviceConfig)")
	...
	assertFileContains(t, "campaigns/composition_service_config.go", "func newServiceConfigFromGRPCDeps(deps campaigngateway.GRPCGatewayDeps, assetBaseURL string) campaignapp.ServiceConfig")
	...
	assertFileContains(t, "campaigns/app/service_config.go", "type ServiceConfig struct")
	assertFileDoesNotContain(t, "campaigns/app/service_config.go", "type AutomationReadServiceConfig struct")
	assertFileDoesNotContain(t, "campaigns/app/service_config.go", "type AutomationMutationServiceConfig struct")
	...
	assertFileContains(t, "campaigns/app/service_ai_binding.go", "func (s automationReadService) campaignAIBindingEditor(")
	...
	assertFileContains(t, "campaigns/handlers_detail_participants.go", "h.automationReads.CampaignAIBindingEditor(ctx, campaignID, page.workspace.AIAgentID)")
	assertFileContains(t, "campaigns/handlers_mutation.go", "h.automationMutate.UpdateCampaignAIBinding(ctx, campaignID, input)")
	assertFileDoesNotContain(t, "campaigns/campaign_page_context.go", "h.workspace.CampaignSessions(")
	assertFileContains(t, "campaigns/campaign_page_context.go", "h.sessionReads.CampaignSessions(")
	assertFileContains(t, "campaigns/handlers_detail_sessions_invites.go", "h.sessionReads.CampaignSessionReadiness(")
	assertFileContains(t, "campaigns/test_config_test.go", "func serviceConfigWithGRPCDeps(")
	assertFileContains(t, "campaigns/test_config_test.go", "return newServiceConfigFromGRPCDeps(deps, assetBaseURL)")
...
func TestRequestResolutionUsesSharedPlatformSeam(t *testing.T) {
	...
	assertFileContains(t, "discovery/handlers.go", "requestresolver.ResolveLocalizedPage(")
	assertFileContains(t, "invite/handlers.go", "requestresolver.ResolveLocalizedPage(")
	assertFileContains(t, "profile/handlers.go", "requestresolver.ResolveLocalizedPage(")
	assertFileContains(t, "publicauth/handlers_passkey.go", "requestresolver.ResolveLocalizedPage(")

...
func (g grpcGateway) CharacterCreationCatalog(ctx context.Context, localeTag language.Tag) (CampaignCharacterCreationCatalog, error) {
	...
	catalog.Classes = make([]CatalogClass, 0, len(catalogResp.GetClasses()))
	for _, class := range catalogResp.GetClasses() {
		...
		catalog.Classes = append(catalog.Classes, CatalogClass{
			ID:        classID,
	...
	catalog.Subclasses = make([]CatalogSubclass, 0, len(catalogResp.GetSubclasses()))
	for _, subclass := range catalogResp.GetSubclasses() {
		...
		catalog.Subclasses = append(catalog.Subclasses, CatalogSubclass{
			ID:      subclassID,
	...
	catalog.Heritages = make([]CatalogHeritage, 0, len(catalogResp.GetHeritages()))
	for _, heritage := range catalogResp.GetHeritages() {
		...
		catalog.Heritages = append(catalog.Heritages, CatalogHeritage{
			ID:   heritageID,
	...
	catalog.Weapons = make([]CatalogWeapon, 0, len(catalogResp.GetWeapons()))
	for _, weapon := range catalogResp.GetWeapons() {
		...
		catalog.Weapons = append(catalog.Weapons, CatalogWeapon{
			ID:       weaponID,
	...
	catalog.Armor = make([]CatalogArmor, 0, len(catalogResp.GetArmor()))
	for _, armor := range catalogResp.GetArmor() {
		...
		catalog.Armor = append(catalog.Armor, CatalogArmor{
			ID:   armorID,
	...
	catalog.Items = make([]CatalogItem, 0, len(catalogResp.GetItems()))
	...
	catalog.DomainCards = make([]CatalogDomainCard, 0, len(catalogResp.GetDomainCards()))
	for _, domainCard := range catalogResp.GetDomainCards() {
		...
		catalog.DomainCards = append(catalog.DomainCards, CatalogDomainCard{
			ID:       domainCardID,
...
func (g grpcGateway) ApplyCharacterCreationStep(ctx context.Context, campaignID string, characterID string, step *CampaignCharacterCreationStepInput) error {
	if g.characterClient == nil {

...
func (g sessionReadGateway) CampaignSessions(ctx context.Context, campaignID string) ([]campaignapp.CampaignSession, error) {
...
func (g sessionMutationGateway) StartSession(ctx context.Context, campaignID string, input campaignapp.StartSessionInput) error {
...
func (g sessionMutationGateway) EndSession(ctx context.Context, campaignID string, input campaignapp.EndSessionInput) error {
	if g.mutation.Session == nil {
		return apperrors.EK(apperrors.KindUnavailable, "error.web.message.session_service_client_is_not_configured", "session service client is not configured")
	}
	campaignID = strings.TrimSpace(campaignID)

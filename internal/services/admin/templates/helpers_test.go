...
func TestAppendQueryParam(t *testing.T) {
	if got := AppendQueryParam("/app/campaigns", "page_token", "abc"); got != "/app/campaigns?page_token=abc" {
		t.Fatalf("expected query param appended, got %q", got)
	}
	if got := AppendQueryParam("/app/campaigns?event_type=foo", "page_token", "a b"); got != "/app/campaigns?event_type=foo&page_token=a+b" {
...
func TestEventFilterBaseURL(t *testing.T) {
	...
	if got := EventFilterBaseURL("/app/campaigns/camp-1/events", filters); got != "/app/campaigns/camp-1/events?event_type=session.started&start_date=2024-02-01" {
		t.Fatalf("expected filters encoded, got %q", got)
	}
	if got := EventFilterBaseURL("/app/campaigns/camp-1/events", EventFilterOptions{}); got != "/app/campaigns/camp-1/events" {

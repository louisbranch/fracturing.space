...
func TestEnsurePrimarySubclassTrack_ReplacesOrAddsPrimaryTrack(t *testing.T) {
	t.Run("adds missing primary", func(t *testing.T) {
		tracks := EnsurePrimarySubclassTrack(nil, "class.guardian", "subclass.stalwart")
		if len(tracks) != 1 {
			t.Fatalf("track count = %d, want 1", len(tracks))
		}
		if tracks[0].Origin != SubclassTrackOriginPrimary || tracks[0].Rank != SubclassTrackRankFoundation {
			t.Fatalf("primary track = %+v", tracks[0])
		}
	})

	t.Run("replaces existing primary and preserves multiclass", func(t *testing.T) {
		tracks := EnsurePrimarySubclassTrack([]CharacterSubclassTrack{
			{Origin: SubclassTrackOriginPrimary, ClassID: "old", SubclassID: "old", Rank: SubclassTrackRankMastery},
			{Origin: SubclassTrackOriginMulticlass, ClassID: "class.bard", SubclassID: "subclass.wordsmith", Rank: SubclassTrackRankFoundation},
		}, "class.guardian", "subclass.stalwart")
		if len(tracks) != 2 {
			t.Fatalf("track count = %d, want 2", len(tracks))
		}
		if tracks[0].ClassID != "class.guardian" || tracks[0].SubclassID != "subclass.stalwart" || tracks[0].Rank != SubclassTrackRankFoundation {
			t.Fatalf("replaced primary = %+v", tracks[0])
		}
		if tracks[1].Origin != SubclassTrackOriginMulticlass {
...
func TestAdvancePrimarySubclassTrack_AdvancesAndStopsAtMastery(t *testing.T) {
	tracks, advanced, err := AdvancePrimarySubclassTrack([]CharacterSubclassTrack{{
	...
	if advanced.Rank != SubclassTrackRankSpecialization || tracks[0].Rank != SubclassTrackRankSpecialization {
		t.Fatalf("advanced track = %+v tracks=%+v", advanced, tracks)
	}

	_, _, err = AdvancePrimarySubclassTrack([]CharacterSubclassTrack{{
		Origin:     SubclassTrackOriginPrimary,
		ClassID:    "class.guardian",
		SubclassID: "subclass.stalwart",
...
func TestAddMulticlassSubclassTrack_RejectsDuplicate(t *testing.T) {
	_, track, err := AddMulticlassSubclassTrack([]CharacterSubclassTrack{{
		Origin:     SubclassTrackOriginPrimary,
		ClassID:    "class.guardian",
		SubclassID: "subclass.stalwart",
		Rank:       SubclassTrackRankFoundation,
	...
	if track.Origin != SubclassTrackOriginMulticlass || track.Rank != SubclassTrackRankFoundation || track.DomainID != "domain.codex" {
		t.Fatalf("multiclass track = %+v", track)
	}

	_, _, err = AddMulticlassSubclassTrack([]CharacterSubclassTrack{{
		Origin:     SubclassTrackOriginPrimary,
		ClassID:    "class.guardian",
		SubclassID: "subclass.stalwart",
		Rank:       SubclassTrackRankFoundation,
...
func TestSubclassTrackHelpers_ErrorAndStageBranches(t *testing.T) {
	t.Run("advance requires primary track", func(t *testing.T) {
		if _, _, err := AdvancePrimarySubclassTrack(nil); err == nil {
			t.Fatal("expected missing primary track error")
		}
	})

	t.Run("loader is required when tracks exist", func(t *testing.T) {
		_, err := ActiveSubclassTrackFeaturesFromLoader(context.Background(), nil, []CharacterSubclassTrack{{
			Origin:     SubclassTrackOriginPrimary,
			ClassID:    "class.guardian",
			SubclassID: "subclass.stalwart",
			Rank:       SubclassTrackRankFoundation,
	...
	t.Run("store is required", func(t *testing.T) {
		_, err := ActiveSubclassTrackFeaturesFromStore(context.Background(), nil, []CharacterSubclassTrack{{
			Origin:     SubclassTrackOriginPrimary,
			ClassID:    "class.guardian",
			SubclassID: "subclass.stalwart",
			Rank:       SubclassTrackRankFoundation,
	...
	t.Run("unlocked features follow requested rank", func(t *testing.T) {
		...
		if got := UnlockedSubclassStageFeatures(subclass, SubclassTrackRankFoundation); len(got) != 1 || got[0].ID != "foundation" {
			t.Fatalf("foundation features = %+v", got)
		}
		if got := UnlockedSubclassStageFeatures(subclass, SubclassTrackRankSpecialization); len(got) != 1 || got[0].ID != "specialization" {
			t.Fatalf("specialization features = %+v", got)
		}
		if got := UnlockedSubclassStageFeatures(subclass, SubclassTrackRankMastery); len(got) != 1 || got[0].ID != "mastery" {
...
func TestActiveSubclassTrackFeaturesFromLoaderAndRuleSummaries(t *testing.T) {
	...
	sets, err := ActiveSubclassTrackFeaturesFromLoader(context.Background(), loader, []CharacterSubclassTrack{
		{Origin: SubclassTrackOriginPrimary, ClassID: "class.guardian", SubclassID: "subclass.stalwart", Rank: SubclassTrackRankMastery},
		{Origin: SubclassTrackOriginMulticlass, ClassID: "class.wizard", SubclassID: "subclass.school-war", Rank: SubclassTrackRankSpecialization},
	})
	if err != nil {
	...
	active := FlattenActiveSubclassFeatures(sets)
	bonuses := SubclassStatBonusesFromFeatures(active)
	...
	profile := &CharacterProfile{HpMax: 6, StressMax: 5, MajorThreshold: 3, SevereThreshold: 6}
	...
	summary := SummarizeActiveSubclassRules(active)
...
func TestSummarizeActiveSubclassRules_PrefersHighestSupportedValues(t *testing.T) {
	...
	summary := SummarizeActiveSubclassRules(features)

...
func recomputeGateProgress(progress *GateProgress, workflow GateWorkflow) {
	...
	if progress.EligibleCount > 0 {
		...
		progress.PendingParticipantIDs = workflowPendingParticipantIDs(progress.EligibleParticipantIDs, respondedSet)
	...
	progress.LeadingOptions, progress.LeadingOptionCount, progress.LeadingTie = workflowLeadingOptions(progress.Options, decisionCounts)
...
func workflowPendingParticipantIDs(eligible []string, responded map[string]struct{}) []string {
	if len(eligible) == 0 {
		return nil
	}
...
func workflowLeadingOptions(options []string, decisionCounts map[string]int) ([]string, int, bool) {
	if len(decisionCounts) == 0 {
		return nil, 0, false
	}
	candidates := options

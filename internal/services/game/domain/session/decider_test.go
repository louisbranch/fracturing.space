...
func TestDecideSessionGateRespond_RejectsEmptyWorkflowResponse(t *testing.T) {
	cmd := command.Command{
		CampaignID:  "camp-1",
		Type:        command.Type("session.gate_record_response"),
		ActorType:   command.ActorTypeSystem,
		SessionID:   "sess-1",
		PayloadJSON: []byte(`{"gate_id":"gate-1","participant_id":"part-1"}`),
	}
...
func TestDecide_MalformedStartPayloadRejected(t *testing.T) {
	decision := Decide(State{}, command.Command{
		CampaignID:  "camp-1",
		Type:        command.Type("session.start"),

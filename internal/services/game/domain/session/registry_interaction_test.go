...
func TestSessionInteractionPayloadValidatorsAcceptAndRejectJSON(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "ooc resumed", validate: func(raw []byte) error { return validateOOCClosedPayload(json.RawMessage(raw)) }, valid: `{"reason":"resume"}`},
		{name: "ooc interruption resolved", validate: func(raw []byte) error { return validateOOCResolvedPayload(json.RawMessage(raw)) }, valid: `{"resolution":"resume_original_phase"}`},

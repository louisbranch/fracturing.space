...
func TestDecideCharacterCreate_NormalizesParticipantID(t *testing.T) {
	now := time.Date(2026, 2, 20, 0, 0, 0, 0, time.UTC)
	cmd := command.Command{
		CampaignID: "camp-1",
		Type:       command.Type("character.create"),
		ActorType:  command.ActorTypeParticipant,
		PayloadJSON: []byte(
			`{"character_id":"char-1","owner_participant_id":"part-owner","participant_id":"  p-1  ","name":"Aria","kind":"PC"}`,
		),
	}
	...
	if payload.ParticipantID != "p-1" {
		t.Fatalf("participant_id = %q, want %q", payload.ParticipantID, "p-1")
	}
}
...
func TestDecideCharacterCreateWithProfile_EmitsCreateAndProfileEvents(t *testing.T) {
	...
	createEvent := decision.Events[0]
	if createEvent.Type != EventTypeCreated {
		t.Fatalf("create event type = %s, want %s", createEvent.Type, EventTypeCreated)
	}
	if !createEvent.Timestamp.Equal(now) {
		t.Fatalf("create event timestamp = %s, want %s", createEvent.Timestamp, now)
	}

	profileEvent := decision.Events[1]
	if profileEvent.Type != EventTypeProfileUpdated {
		t.Fatalf("profile event type = %s, want %s", profileEvent.Type, EventTypeProfileUpdated)
	}
	if profileEvent.ActorType != event.ActorType(command.ActorTypeGM) {
		t.Fatalf("profile event actor_type = %s, want %s", profileEvent.ActorType, event.ActorType(command.ActorTypeGM))
	}
	if profileEvent.ActorID != "user-1" {
		t.Fatalf("profile event actor_id = %q, want %q", profileEvent.ActorID, "user-1")
	}
	if !profileEvent.Timestamp.Equal(now) {
		t.Fatalf("profile event timestamp = %s, want %s", profileEvent.Timestamp, now)
	}

	var profilePayload ProfileUpdatePayload
	if err := json.Unmarshal(profileEvent.PayloadJSON, &profilePayload); err != nil {
		t.Fatalf("decode profile payload: %v", err)
	}
	if profilePayload.CharacterID != "char-1" {
		t.Fatalf("character_id = %q, want %q", profilePayload.CharacterID, "char-1")
	}
	if _, ok := profilePayload.SystemProfile["daggerheart"]; !ok {
		t.Fatalf("expected daggerheart profile key, got %v", profilePayload.SystemProfile)
	}
}

func TestDecideCharacterCreateWithProfile_WithoutSystemProfileReturnsCreateOnly(t *testing.T) {
	decision := Decide(State{}, command.Command{
		CampaignID: "camp-1",
		Type:       command.Type("character.create_with_profile"),
		ActorType:  command.ActorTypeSystem,
	...
	if decision.Events[0].Type != EventTypeCreated {
		t.Fatalf("event type = %s, want %s", decision.Events[0].Type, EventTypeCreated)
	}
}

func TestDecideCharacterCreateWithProfile_CreateValidationRejected(t *testing.T) {
	decision := Decide(State{}, command.Command{
		CampaignID: "camp-1",
		Type:       command.Type("character.create_with_profile"),
		ActorType:  command.ActorTypeSystem,
		PayloadJSON: []byte(`{
			"create":{"character_id":" ","name":"Aria","kind":"PC"},
			"system_profile":{"daggerheart":{}}
		}`),
	}, nil)
	if len(decision.Events) != 0 {
		t.Fatalf("expected no events, got %d", len(decision.Events))
...
func TestDecideCharacterCreateWithProfile_MalformedPayloadRejected(t *testing.T) {
	decision := Decide(State{}, command.Command{
		CampaignID:  "camp-1",
		Type:        command.Type("character.create_with_profile"),
		PayloadJSON: []byte(`{`),

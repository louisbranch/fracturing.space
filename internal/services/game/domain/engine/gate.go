...
// CheckScene returns a rejection when a scene gate blocks the command.
func (g DecisionGate) CheckScene(state scene.State, cmd command.Command) command.Decision {
	if g.Registry == nil {
		return command.Decision{}
	}
	def, ok := g.Registry.Definition(cmd.Type)
	...
	if def.Gate.Scope != command.GateScopeScene {
		return command.Decision{}
	}
	if def.Gate.AllowWhenOpen || !state.GateOpen {
		return command.Decision{}

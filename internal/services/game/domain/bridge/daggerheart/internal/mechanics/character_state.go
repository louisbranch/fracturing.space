...
func normalizeLifeState(cfg CharacterStateConfig) CharacterStateConfig {
	normalized := strings.TrimSpace(cfg.LifeState)
	if normalized == "" {
		normalized = LifeStateAlive
	}
	cfg.LifeState = normalized
	return cfg
}

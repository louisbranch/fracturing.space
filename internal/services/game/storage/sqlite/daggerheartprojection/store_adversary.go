...
func (s *Store) PutDaggerheartAdversary(ctx context.Context, adversary projectionstore.DaggerheartAdversary) error {
	...
	if err := requireProjectionField(adversary.CampaignID, "campaign id"); err != nil {
		return err
	}
	if err := requireProjectionField(adversary.AdversaryID, "adversary id"); err != nil {
		return err
	}
	if err := requireProjectionField(adversary.AdversaryEntryID, "adversary entry id"); err != nil {
		return err
	}
	if err := requireProjectionField(adversary.Name, "adversary name"); err != nil {
		return err
	}
	if err := requireProjectionField(adversary.SessionID, "session id"); err != nil {
		return err
	}
...
func (s *Store) GetDaggerheartAdversary(ctx context.Context, campaignID, adversaryID string) (projectionstore.DaggerheartAdversary, error) {
	...
	if err := requireProjectionField(adversaryID, "adversary id"); err != nil {
		return projectionstore.DaggerheartAdversary{}, err
	}
	...
	return dbDaggerheartAdversaryToDomain(row)
}
...
func dbDaggerheartAdversaryToDomain(row db.DaggerheartAdversary) (projectionstore.DaggerheartAdversary, error) {
	conditions := []projectionstore.DaggerheartConditionState{}

...
func TestContentStoreNilErrors(t *testing.T) {
	...
	if err := s.PutDaggerheartClass(ctx, contentstore.DaggerheartClass{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartSubclass(ctx, contentstore.DaggerheartSubclass{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartHeritage(ctx, contentstore.DaggerheartHeritage{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartExperience(ctx, contentstore.DaggerheartExperienceEntry{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartAdversaryEntry(ctx, contentstore.DaggerheartAdversaryEntry{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartBeastform(ctx, contentstore.DaggerheartBeastformEntry{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartCompanionExperience(ctx, contentstore.DaggerheartCompanionExperienceEntry{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartLootEntry(ctx, contentstore.DaggerheartLootEntry{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartDamageType(ctx, contentstore.DaggerheartDamageTypeEntry{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartDomain(ctx, contentstore.DaggerheartDomain{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartDomainCard(ctx, contentstore.DaggerheartDomainCard{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartWeapon(ctx, contentstore.DaggerheartWeapon{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartArmor(ctx, contentstore.DaggerheartArmor{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartItem(ctx, contentstore.DaggerheartItem{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartEnvironment(ctx, contentstore.DaggerheartEnvironment{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := s.PutDaggerheartContentString(ctx, contentstore.DaggerheartContentString{ContentID: "x", ContentType: "t", Field: "f", Locale: "en"}); err == nil {
...
func TestContentStoreCancelledContextErrors(t *testing.T) {
	...
	if err := store.PutDaggerheartClass(ctx, contentstore.DaggerheartClass{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartSubclass(ctx, contentstore.DaggerheartSubclass{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartHeritage(ctx, contentstore.DaggerheartHeritage{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartExperience(ctx, contentstore.DaggerheartExperienceEntry{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartAdversaryEntry(ctx, contentstore.DaggerheartAdversaryEntry{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartBeastform(ctx, contentstore.DaggerheartBeastformEntry{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartCompanionExperience(ctx, contentstore.DaggerheartCompanionExperienceEntry{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartLootEntry(ctx, contentstore.DaggerheartLootEntry{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartDamageType(ctx, contentstore.DaggerheartDamageTypeEntry{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartDomain(ctx, contentstore.DaggerheartDomain{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartDomainCard(ctx, contentstore.DaggerheartDomainCard{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartWeapon(ctx, contentstore.DaggerheartWeapon{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartArmor(ctx, contentstore.DaggerheartArmor{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartItem(ctx, contentstore.DaggerheartItem{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartEnvironment(ctx, contentstore.DaggerheartEnvironment{ID: "x", CreatedAt: now, UpdatedAt: now}); err == nil {
	...
	if err := store.PutDaggerheartContentString(ctx, contentstore.DaggerheartContentString{ContentID: "x", ContentType: "t", Field: "f", Locale: "en"}); err == nil {
...
func TestContentStoreEmptyIDValidation(t *testing.T) {
	...
	if err := store.PutDaggerheartClass(ctx, contentstore.DaggerheartClass{}); err == nil {
	...
	if err := store.PutDaggerheartSubclass(ctx, contentstore.DaggerheartSubclass{}); err == nil {
	...
	if err := store.PutDaggerheartHeritage(ctx, contentstore.DaggerheartHeritage{}); err == nil {
	...
	if err := store.PutDaggerheartExperience(ctx, contentstore.DaggerheartExperienceEntry{}); err == nil {
	...
	if err := store.PutDaggerheartAdversaryEntry(ctx, contentstore.DaggerheartAdversaryEntry{}); err == nil {
	...
	if err := store.PutDaggerheartBeastform(ctx, contentstore.DaggerheartBeastformEntry{}); err == nil {
	...
	if err := store.PutDaggerheartWeapon(ctx, contentstore.DaggerheartWeapon{}); err == nil {
	...
	if err := store.PutDaggerheartArmor(ctx, contentstore.DaggerheartArmor{}); err == nil {
	...
	if err := store.PutDaggerheartItem(ctx, contentstore.DaggerheartItem{}); err == nil {
	...
	if err := store.PutDaggerheartEnvironment(ctx, contentstore.DaggerheartEnvironment{}); err == nil {
	...
	if err := store.PutDaggerheartDomainCard(ctx, contentstore.DaggerheartDomainCard{}); err == nil {
	...
	if err := store.PutDaggerheartCompanionExperience(ctx, contentstore.DaggerheartCompanionExperienceEntry{}); err == nil {
	...
	if err := store.PutDaggerheartLootEntry(ctx, contentstore.DaggerheartLootEntry{}); err == nil {
	...
	if err := store.PutDaggerheartDamageType(ctx, contentstore.DaggerheartDamageTypeEntry{}); err == nil {

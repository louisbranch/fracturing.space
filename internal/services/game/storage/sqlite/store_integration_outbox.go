...
func enqueueIntegrationOutboxEvent(ctx context.Context, exec integrationOutboxExec, outboxEvent storage.IntegrationOutboxEvent) error {
...
func (s *Store) enqueueIntegrationOutboxForEvent(ctx context.Context, tx *sql.Tx, evt event.Event) error {
	outboxEvents, err := integrationOutboxEventsForEvent(evt)
	if err != nil {
		return err
	}
	for _, outboxEvent := range outboxEvents {
		if err := enqueueIntegrationOutboxEvent(ctx, tx, outboxEvent); err != nil {
			return err
		}
	}
	return nil
...
func (s *Store) EnqueueIntegrationOutboxEvent(ctx context.Context, outboxEvent storage.IntegrationOutboxEvent) error {
	...
	return enqueueIntegrationOutboxEvent(ctx, s.sqlDB, outboxEvent)
}

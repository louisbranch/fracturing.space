...
func (h *Handler) ApplyRest(ctx context.Context, in *pb.DaggerheartApplyRestRequest) (RestResult, error) {
	...
	countdownsByID := make(map[ids.CountdownID]daggerheart.Countdown)
	longTermCountdownID := ids.CountdownID(strings.TrimSpace(in.Rest.GetLongTermCountdownId()))
...
func affectedRestCharacterIDs(payload daggerheart.RestTakePayload) []string {

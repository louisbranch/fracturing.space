...
func (a interactionApplication) StartScenePlayerPhase(ctx context.Context, campaignID string, in *campaignv1.StartScenePlayerPhaseRequest) (*campaignv1.InteractionState, error) {
	...
	if err := a.executeSceneCommand(ctx, commandTypeSceneGMInteractionCommit, campaignID, activeSession.ID, sceneID, interactionPayload, "scene.gm_interaction.commit"); err != nil {
		return nil, err
	}
...
func (a interactionApplication) CommitSceneGMInteraction(ctx context.Context, campaignID string, in *campaignv1.CommitSceneGMInteractionRequest) (*campaignv1.InteractionState, error) {
	...
	payload, err := a.buildGMInteractionPayload(in.GetInteraction(), ids.SceneID(sceneID), currentSceneInteraction.PhaseID, ids.ParticipantID(actor.ID))
	if err != nil {
		return nil, err
	}
	if err := a.executeSceneCommand(ctx, commandTypeSceneGMInteractionCommit, campaignID, activeSession.ID, sceneID, payload, "scene.gm_interaction.commit"); err != nil {
		return nil, err
	}
	return a.GetInteractionState(ctx, campaignID)
}
...
func (a interactionApplication) ResolveScenePlayerPhaseReview(ctx context.Context, campaignID string, in *campaignv1.ResolveScenePlayerPhaseReviewRequest) (*campaignv1.InteractionState, error) {
	...
	case *campaignv1.ResolveScenePlayerPhaseReviewRequest_AdvanceToPlayers:
		...
		interactionPayload, err := a.buildGMInteractionPayload(advance.GetInteraction(), ids.SceneID(sceneID), nextPhaseID, ids.ParticipantID(actor.ID))
		if err != nil {
			return nil, err
		}
		if err := a.executeSceneCommand(ctx, commandTypeSceneGMInteractionCommit, campaignID, activeSession.ID, sceneID, interactionPayload, "scene.gm_interaction.commit"); err != nil {
	...
	case *campaignv1.ResolveScenePlayerPhaseReviewRequest_RequestRevisions:
		...
		interactionPayload, err := a.buildGMInteractionPayload(request.GetInteraction(), ids.SceneID(sceneID), currentSceneInteraction.PhaseID, ids.ParticipantID(actor.ID))
		if err != nil {
			return nil, err
		}
		if err := a.executeSceneCommand(ctx, commandTypeSceneGMInteractionCommit, campaignID, activeSession.ID, sceneID, interactionPayload, "scene.gm_interaction.commit"); err != nil {
	...
	case *campaignv1.ResolveScenePlayerPhaseReviewRequest_ReturnToGm:
		...
		interactionPayload, err := a.buildGMInteractionPayload(returnToGM.GetInteraction(), ids.SceneID(sceneID), currentSceneInteraction.PhaseID, ids.ParticipantID(actor.ID))
		if err != nil {
			return nil, err
		}
		if err := a.executeSceneCommand(ctx, commandTypeSceneGMInteractionCommit, campaignID, activeSession.ID, sceneID, interactionPayload, "scene.gm_interaction.commit"); err != nil {
		...
		if err := a.clearOOCResolutionIfPending(ctx, campaignID, activeSession.ID, currentSessionInteraction, "review_returned_to_gm"); err != nil {
			return nil, err
		}
	default:

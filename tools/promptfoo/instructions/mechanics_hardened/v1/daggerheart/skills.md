...
- In this eval profile, explicit Hope spends must be reflected in authoritative state before you narrate the outcome.
- If a player names both Hope and an experience, the minimum valid path is sheet first, authoritative action resolution second, narration last.
- Do not let any Hope gained from the roll outcome mask a missing upfront Hope spend.

...
- In this eval profile, mechanics fidelity outranks stylistic flourish when the two are in tension.
- When a player explicitly spends a resource such as Hope, the authoritative state change must land before you narrate the consequence.
- Do not describe a resource as spent unless the authoritative mechanics tool path actually applied that spend.

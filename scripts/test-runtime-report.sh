...
case "$MODE" in
smoke)
	run_capture integration-smoke go test -count=1 -tags=integration ./internal/test/integration -run "$INTEGRATION_SMOKE_FULL_PATTERN" -json
	...
	;;
smoke-pr)
	run_capture integration-smoke-pr go test -count=1 -tags=integration ./internal/test/integration -run "$INTEGRATION_SMOKE_PR_PATTERN" -json
	run_capture scenario-smoke env SCENARIO_MANIFEST="$SCENARIO_SMOKE_MANIFEST" go test -count=1 -parallel="$SCENARIO_PARALLELISM" -tags=scenario ./internal/test/game -json
	;;
integration-full)
	run_capture integration-full go test -count=1 -tags=integration ./internal/test/integration -json
	;;
integration-shard)

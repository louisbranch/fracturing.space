...
if [[ -n "${WEB_SMOKE_AUTH_ADDR:-}" && -z "${WEB_SMOKE_SESSION_ID:-}" ]]; then
  session_env="$(go run ./internal/tools/websmokeauth \
    ...
    -email-prefix "${WEB_SMOKE_EMAIL_PREFIX:-web-smoke}")"
...
Set WEB_SMOKE_AUTH_ADDR to mint an ephemeral session, or set WEB_SMOKE_SESSION_ID directly.
...
Set WEB_SMOKE_AUTH_ADDR to auto-mint identities, or set WEB_SMOKE_RECIPIENT_USER_ID (or legacy WEB_SMOKE_USER_ID) directly.
EOF
  exit 1
fi

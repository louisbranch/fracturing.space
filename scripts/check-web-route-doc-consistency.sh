...
repo = Path('.').resolve()
architecture_doc = repo / 'docs' / 'architecture' / 'architecture.md'
web_architecture_doc = repo / 'docs' / 'architecture' / 'web-architecture.md'
...
if not line_match:
    ...
    print('  docs/architecture/architecture.md: missing canonical authenticated surface line.', file=sys.stderr)
    sys.exit(1)

routes_clause = line_match.group(1)
if '/app/invites' in routes_clause:
    print('Web route docs consistency check failed:', file=sys.stderr)
    print('  docs/architecture/architecture.md: authenticated surface still includes `/app/invites`, which is intentionally unregistered.', file=sys.stderr)
    sys.exit(1)
...
if missing_required:
    print('Web route docs consistency check failed:', file=sys.stderr)
    print('  docs/architecture/architecture.md: authenticated surface line is missing expected route claims:', file=sys.stderr)
...
if web_invariant not in web_architecture_text:
    print('Web route docs consistency check failed:', file=sys.stderr)
    print('  docs/architecture/web-architecture.md: missing explicit `/app/invites` unregistered invariant.', file=sys.stderr)
    sys.exit(1)
